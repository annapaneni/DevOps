...
Try
{
    ...
    # Get load balancer.
    $nlb= Get-AzLoadBalancer -ResourceGroupName $rgName -Name $lbName 
    if($nlb -eq $null){
    ...
    $nicDbSrv = Get-AzNetworkInterface -Name $dbServerNic -ResourceGroupName $rgName 
    if($nicDbSrv -eq $null){
    ...
    $nicAppSrv = Get-AzNetworkInterface -Name $appServerNic -ResourceGroupName $rgName   
    ...
    $availSet = Get-AzAvailabilitySet -ResourceGroupName $rgName -Name $availabilitySet
    if(!$availSet){
    ...
    $dbVM = Get-AzVM -ResourceGroupName $rgName -Name $dbServerVM   
    if($dbVM -eq $null){
    ...
    $appVM = Get-AzVM -ResourceGroupName $rgName -Name $appServerVM    
    if($appVM -eq $null){
