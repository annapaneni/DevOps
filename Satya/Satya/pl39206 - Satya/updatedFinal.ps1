$rgName = 'Midlandls-Core_RG_test95'
$location= 'EastUS2'
$snetname= 'Midlandls_Core_Subnet_test95'
$snetAddressPrefix= '10.4.1.0/24'
$vnetname= 'Midlandls_Core_VNET_test95'
$vnetAddressPrefix= '10.4.0.0/23'
...
$webServerIPAddress = '10.4.1.6'
$dbServerIPAddress = '10.4.1.7'
$appServerIPAddress = '10.4.1.8'
...
Try
{
    ...
    if($nlb -eq $null){
        ...
        if($frontendIP -ne $null){
            Write-Host "Front end ip config is created"
        }
        ...
        if($beAddressPool -ne $null){
            Write-Host "Back end address pool is created"
        }
        ...
        if($healthProbe -ne $null){
            Write-Host "Health probe is created "
        }
        ...
        if($lbRule -ne $null){
            Write-Host "Load balancer rule is created"
        }        
        ...
        Write-Host "Load balancer is created "
        
    } 
    else
    {
    Write-Host $nlb.Name "lb already exists."
    }
    ...
    if($nicWebSrv -eq $null){
        $nicWebSrv = createNIC -rgName $rgName -location $location -name $webServerNic -publicIp $null -privateIp $webServerIPAddress -nsg $nsg -subnet $vnet.Subnets[0]
    ...
    if($nicDbSrv -eq $null){
        $nicDbSrv = createNIC -rgName $rgName -location $location -name $dbServerNic -publicIp $null -privateIp $dbServerIPAddress -nsg $nsg -subnet $vnet.Subnets[0]
    ...
    if($nicAppSrv -eq $null){
        $nicAppSrv = createNIC -rgName $rgName -location $location -name $appServerNic -publicIp $null -privateIp $appServerIPAddress -nsg $nsg -subnet $vnet.Subnets[0]
